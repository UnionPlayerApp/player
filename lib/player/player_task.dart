...
class PlayerTask extends BackgroundAudioTask {
  ...
  Future<void> _broadcastScheduleState(ScheduleRepositoryState state) async {
    if (state is ScheduleRepositoryLoadSuccessState) {
      ...
      var queue = <MediaItem>[];
      await Future.forEach(state.items, (ScheduleItem item) async {
        queue.add(await _mapScheduleItemToMediaItem(item));
      });
      print("QUEUE: $queue");
  ...
  Future<MediaItem> _mapScheduleItemToMediaItem(ScheduleItem scheduleItem) async {
    ...
    switch (scheduleItem.type) {
      ...
      case ScheduleItemType.music:
        print("scheduleItem.imageUrl = ${scheduleItem.imageUrl}");
        if (scheduleItem.imageUrl == null || scheduleItem.imageUrl == ""){
          artUri = _getRandomUri(_musicArtUriList);
        } else {
          var artFile = await loadRemoteFile(scheduleItem.imageUrl!);
          artUri = artFile.uri;
        }
        break;
